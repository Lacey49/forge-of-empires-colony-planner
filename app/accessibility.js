...
(() => {
  ...
  function describeCell(cell) {
    ...
    let detail = "";
    try {
      if (typeof placedThingTooltip === "function")
        detail = placedThingTooltip(r, c) || "";
    } catch {}
  ...
  function decorateBoard(restoreFocus = false) {
    ...
    board.setAttribute(
      "aria-label",
      "Colony planning grid. Use arrow keys to move, Enter or Space to place or select, and Delete to remove.",
    );
  ...
  render = function (...args) {
    ...
    const restoreFocus =
      !!active?.classList?.contains("cell") && active.parentElement === board;
    if (restoreFocus) {
      focusPos = { r: Number(active.dataset.r), c: Number(active.dataset.c) };
    }
