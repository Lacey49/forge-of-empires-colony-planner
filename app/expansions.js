...
(() => {
  if (window.__FOE_STATE_CONSISTENCY_V2__) return;
  window.__FOE_STATE_CONSISTENCY_V2__ = true;
  ...
  function cellsForBuildingRecord(building) {
    if (Array.isArray(building?.cells) && building.cells.length)
      return building.cells;
    if (building && typeof cellsFor === "function")
      return cellsFor(building.type, building.r, building.c);
    return [];
  }
  ...
  function hallFitsAt(top, occupied) {
    ...
    for (const [r, c] of hubCells(top[0], top[1])) {
      if (r < 0 || r >= 28 || c < 0 || c >= 28 || !ownedCell(r, c))
        return false;
  ...
  function chooseSafeHallPosition() {
    ...
    const preferred = Array.isArray(cfg?.defaultHub)
      ? [...cfg.defaultHub]
      : null;
    ...
    let best = null,
      bestScore = Infinity;
    const pr = preferred?.[0] ?? hubTop[0],
      pc = preferred?.[1] ?? hubTop[1];
  ...
  function removeBuildingsOverlapping(cellSet) {
    const kept = [];
    for (const building of buildings) {
      const hit = cellsForBuildingRecord(building).some(([r, c]) =>
        cellSet.has(r + "," + c),
      );
      if (!hit) {
        kept.push(building);
  ...
  function sanitizeAfterLandChange() {
    ...
    for (const building of buildings) {
      ...
      const valid =
        cells.length &&
        cells.every(
          ([r, c]) => r >= 0 && r < 28 && c >= 0 && c < 28 && ownedCell(r, c),
        );
      if (valid) kept.push(building);
      else clearBuildingRecord(building);
    }
    buildings = kept;
    ...
    if (!currentHallCells.every(([r, c]) => ownedCell(r, c))) {
      ...
      removeBuildingsOverlapping(nextSet);

      for (const [r, c] of nextCells) grid[r][c] = cellStateBase(r, c);
      hubTop = [...nextHub];
      applyHub();
    }
