...
(() => {
  ...
  // Footprints used by older saves, before the in-game orientation corrections.
  const previous = {
    heatedResidence: {
      w: 3,
      h: 4,
    },
    matterCompressionReactor: {
      w: 6,
      h: 4,
    },
    moleculeDrill: {
  ...
  function matchesFootprint(building, dims) {
    ...
    return (
      ...
      expected.every(([r, c]) => actual.has(`${r},${c}`))
    );
  }
  ...
  function migrateState(state) {
    ...
    const targets = state.buildings.filter(
      (building) =>
    ...
    for (const building of targets) {
      ...
      for (let r = 0; r <= 28 - next.h; r++) {
        for (let c = 0; c <= 28 - next.w; c++) {
          ...
          const score =
            Math.abs(centerR - oldCenterR) + Math.abs(centerC - oldCenterC);
          const anchorPenalty =
            Math.abs(r - building.r) + Math.abs(c - building.c);

          if (
            !best ||
            score < best.score ||
            (score === best.score && anchorPenalty < best.anchorPenalty)
          ) {
  ...
  if (slot) {
    ...
    if (workspaceChanged && typeof saveWorkspace === "function")
      saveWorkspace();
  }
