...
(() => {
  ...
  function neighborhoodCenters(ctx,state){
    ...
    for(const p of candidates){
      if(out.some(q=>Math.abs(q.r-p.r)+Math.abs(q.c-p.c)<5))continue;
      out.push(p);if(out.length>=18)break;
    }
    return out;
  }
  ...
  function windowRoadVariant(ctx,sol,primary,center,size,kind,phase){
    const half=Math.floor(size/2),r0=Math.max(0,center.r-half),r1=Math.min(27,center.r+half),c0=Math.max(0,center.c-half),c1=Math.min(27,center.c+half);
    ...
    const rs=primary.h*2+1,cs=primary.w*2+1;
    const rowPhase=mod(center.r+phase,rs),colPhase=mod(center.c+phase,cs);
    const addRows=()=>{
      for(let r=r0;r<=r1;r++)if(mod(r-rowPhase,rs)===0)for(let c=c0;c<=c1;c++)addRoadIfValid(ctx,sol.hall,roads,r,c);
    };
    const addCols=()=>{
      for(let c=c0;c<=c1;c++)if(mod(c-colPhase,cs)===0)for(let r=r0;r<=r1;r++)addRoadIfValid(ctx,sol.hall,roads,r,c);
    };
    ...
    if(kind==='h'){
      ...
      for(let r=r0;r<=r1;r++)addRoadIfValid(ctx,sol.hall,roads,r,tc);
    }else if(kind==='v'){
      addCols();
      const tr=trunkRows[phase%trunkRows.length];
      for(let c=c0;c<=c1;c++)addRoadIfValid(ctx,sol.hall,roads,tr,c);
    }else if(kind==='grid'){
      addRows();addCols();
    }else if(kind==='frame'){
      for(let c=c0;c<=c1;c++){addRoadIfValid(ctx,sol.hall,roads,r0,c);addRoadIfValid(ctx,sol.hall,roads,r1,c)}
      for(let r=r0;r<=r1;r++){addRoadIfValid(ctx,sol.hall,roads,r,c0);addRoadIfValid(ctx,sol.hall,roads,r,c1)}
    }
    return normalizeRoads(ctx,sol.hall,roads);
  }

  async function rebuildNeighborhoods(era,goal,primaryKey,initialResult,overallStart,deadline){
    if(goal!=='maxCredits'||optimizerCancelRequested||performance.now()>=deadline)return initialResult;
    const ctx=oxCtx(era);ctx.goal=goal;ctx.primaryKey=primaryKey;ctx.started=overallStart;ctx.deadline=deadline;ctx.tested=initialResult.tested||0;ctx.bestState=cloneState(initialResult.state);ctx.bestScore=oxScore(ctx,ctx.bestState,primaryKey);
    const primary=eraBoardBuildingByKey(era,primaryKey),allowedKeys=oxAllowedResidentialKeys(era,primaryKey);
    ...
    const accept=sol=>{
      ...
      ctx.bestState=state;ctx.bestScore=score;oxProgress(ctx,'Rebuilding tight spots');return true;
    };

    let round=0;
    while(!optimizerCancelRequested&&performance.now()<deadline-100&&round<4){
      round++;
      const reference=oxSolFromState(ctx,ctx.bestState);if(!reference)break;
        ...
        for(const size of sizes){
          for(const kind of kinds){
            ...
            for(let phase=0;phase<phaseLimit;phase++){
              if(optimizerCancelRequested||performance.now()>=deadline-100)break;
              const network=windowRoadVariant(ctx,reference,primary,center,size,kind,phase);
              if(network){
                const sol=oxPack(ctx,reference.hall,network,primary,goal,'normal',oxHash((round+1)*1000003+job*8191+ctx.tested));
                if(accept(sol))improved=true;
              }
              job++;
              await new Promise(resolve=>setTimeout(resolve,0));
              if(performance.now()-ctx.lastProgress>120)oxProgress(ctx,'Rebuilding tight spots');
            }
            if(optimizerCancelRequested||performance.now()>=deadline-100)break;
          }
          if(optimizerCancelRequested||performance.now()>=deadline-100)break;
  ...
  optimizeColonyV2=async function(era,goal,primaryKey,mode){
    ...
    return rebuildNeighborhoods(era,goal,primaryKey,result,overallStart,deadline);
  };
  ...
  if(previousSyncLabels){
    optimizerSyncSearchLabels=function(){
      ...
      if(!select)return;
      for(const option of select.options)if(option.value==='deep')option.textContent='Deep · ~120s';
    };
  }
})();