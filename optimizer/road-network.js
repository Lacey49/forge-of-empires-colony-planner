...
(() => {
  ...
  function targetMetric(strategy, distance, coverage, edge, id, step) {
    ...
    const jitter =
      oxHash(id ^ ((strategy + 1) * 104729) ^ ((step + 1) * 8191)) % 170;
  ...
  function rebuildRoadNetwork(ctx, sol, strategy) {
    ...
    while (remaining > 0 && step < groups.length + 8) {
      ...
      for (const [id, cover] of coverage) {
        ...
        if (
          metric < bestMetric ||
          (metric === bestMetric && (best == null || id < best))
        ) {
          bestMetric = metric;
          best = id;
        }
    ...
    if (
      work.roads.size &&
      oxConnected(work.roads, work.hall.set).size !== work.roads.size
    )
      return null;
    return oxPrune(ctx, work);
  }

  async function compactPaths(
    era,
    goal,
    primaryKey,
    initialResult,
    ...
    while (
      ...
      if (compacted) {
        ...
        const state = oxState(ctx, compacted),
  ...
  if (previousSyncLabels) {
    optimizerSyncSearchLabels = function () {
      ...
      for (const option of select.options)
        if (option.value === "deep") option.textContent = "Deep · ~150s";
    };
  }
})();
