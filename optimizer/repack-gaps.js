...
(() => {
  ...
  oxPack = function (ctx, hall, roads, primary, goal, mode, seed) {
    ...
    const fillerPlacements = fillers.map((d) => ({
      d,
      placements: oxPlacements(ctx, d, hall, roads),
    }));
    ...
    for (let s = 0; s < starts; s++) {
      ...
      for (const item of fillerPlacements) {
        for (const p of oxSort(
          item.placements,
          order,
          seed + s * 104729 + item.d.key.length,
        )) {
          if ((p.mask & blocked) !== 0n) continue;
          placed.push(p);
          blocked |= p.mask;
      ...
      const state = oxState(ctx, sol),
        score = oxScore(ctx, state, ctx.primaryKey);
      ctx.tested++;
  ...
  function lateCandidatePositions(ctx, hall, def, referenceSol) {
    ...
    for (const p of referenceSol.placements)
      for (const id of p.ids) currentBuildingCells.add(id);
    ...
    for (let r = 0; r <= 28 - def.h; r++) {
      for (let c = 0; c <= 28 - def.w; c++) {
        const ids = oxRect(r, c, def.h, def.w),
          mask = oxMask(ids);
        if ((mask & ~ctx.ownedMask) !== 0n || (mask & hall.mask) !== 0n)
          continue;
        let roadHits = 0,
          buildingHits = 0;
        for (const id of ids) {
        ...
        const centerR = r + def.h / 2,
          centerC = c + def.w / 2;
        const hallDistance =
          Math.abs(centerR - hallCenterR) + Math.abs(centerC - hallCenterC);
        out.push({
          type: def.key,
          r,
          c,
  ...
  async function fillFreedGapsResponsive(
    ctx,
    sol,
    primary,
    fillers,
    label = "Repacking gaps",
  ) {
    ...
    for (
      ...
      for (const d of [primary, ...fillers]) {
        ...
        for (const p of placements) {
          ...
          if ((++checked & 31) === 0 && !(await uiYield(ctx, label)))
            return work;
        }
      }
      if (!added) break;
  ...
  async function repairRoadNetworkResponsive(
    ctx,
    sol,
    label = "Repacking gaps",
  ) {
  ...
  async function packResponsive(
    ...
    for (
      ...
      if (
        !oxAccess(ctx, sol.placements, sol.roads) ||
        oxConnected(sol.roads, hall.set).size !== sol.roads.size
      ) {
  ...
  function roadShiftVariants(ctx, sol) {
    ...
    const mutateRow = (row, delta) => {
      ...
      if (nr >= 0 && nr < 28)
        for (let c = 0; c < 28; c++) {
          const id = optId(nr, c);
          if (ctx.owned.has(id) && !sol.hall.set.has(id)) roads.add(id);
        }
      variants.push(roads);
    };
  ...
  async function lateRepack(
    ...
    ctx.lastYield = performance.now();
    ctx.lastProgress = performance.now();
    ...
    while (
      ...
      const candidates = lateCandidatePositions(
        ctx,
        reference.hall,
        primary,
        reference,
      ...
      for (
        ...
        const sol = await packResponsive(
          ...
          primary,
          goal,
          oxHash((round + 7) * 65537 + i * 104729 + ctx.tested),
          null,
          8,
        );
        if (accept(sol)) improvedThisRound = true;
      ...
      if (!improvedThisRound && performance.now() < deadline - 100) {
        const shuffled = candidates
          ...
          .sort(
            (a, b) =>
              oxHash(optId(a.r, a.c) + ctx.tested) -
              oxHash(optId(b.r, b.c) + ctx.tested),
          );
        for (
          let i = 0;
          i < shuffled.length && performance.now() < deadline - 100;
          i++
        ) {
          if (!(await uiYield(ctx, "Repacking gaps"))) break;
          const forced = shuffled[i],
            roads = new Set(refreshed.roads);
          for (const id of forced.ids) roads.delete(id);
          ...
          const sol = await packResponsive(
            ...
            primary,
            goal,
            oxHash((round + 19) * 99991 + i + ctx.tested),
            forced,
            6,
          );
          if (accept(sol)) improvedThisRound = true;
