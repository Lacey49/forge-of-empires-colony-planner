...
(() => {
  if (typeof optimizeColonyV2 !== 'function' || typeof oxFillFreedGaps !== 'function') return;
  ...
  function promoteOnce(ctx, sol, primary, fillers) {
    ...
    for (let r=0; r<=28-primary.h; r++) {
      for (let c=0; c<=28-primary.w; c++) {
        ...
        if ((mask & ~ctx.ownedMask) !== 0n || (mask & sol.hall.mask) !== 0n) continue;
        ...
        if (
          ...
          (candidate.gain === best.gain && candidate.removedCount < best.removedCount) ||
          (candidate.gain === best.gain && candidate.removedCount === best.removedCount && (candidate.r < best.r || (candidate.r === best.r && candidate.c < best.c)))
        ) best = candidate;
      }
    }
    ...
    return {
      sol:{hall:sol.hall, roads:new Set(sol.roads), placements:nextPlacements},
      changed:true
    };
  }
  ...
  function promoteFillers(ctx, sol, primary, fillers) {
    ...
    let work = {hall:sol.hall, roads:new Set(sol.roads), placements:sol.placements.map(clonePlacement)};
  ...
  optimizeColonyV2 = async function(era, goal, primaryKey, mode) {
    ...
    const fillers = oxAllowedResidentialDefs(era,primaryKey).filter(d => d.key !== primaryKey);
