...
(() => {
  ...
  const SAM_SIMPLE_SHELTER_ALL_SHELTERS = [
    ...
    [4,0],[4,10],[4,13],[4,16],[4,19],[4,22],[4,25],
    [7,0],[7,10],[7,13],[7,16],[7,19],[7,22],[7,25],
    [10,0],[10,4],
    [11,7],[11,10],[11,13],[11,16],[11,19],[11,22],[11,25],
    [13,0],[13,4],
    [14,7],[14,10],[14,13],[14,16],[14,19],[14,22],[14,25],
    [16,0],
    [18,4],[18,7],[18,10],[18,13],[18,16],[18,19],[18,22],[18,25],
    [19,0],
    [21,5],[21,8],[21,11],[21,14],[21,17],[21,20],[21,23],
    [25,4],[25,7],[25,10],[25,13],[25,16],[25,19],[25,22],[25,25]
  ];
  const SAM_SIMPLE_SHELTER_ALL_DROP_PODS = [[22,1],[23,26]];
  const SAM_SIMPLE_SHELTER_ALL_PATHS = [
    [3,6],[3,9],[3,10],[3,11],[3,12],[3,13],[3,14],[3,15],[3,16],[3,17],[3,18],[3,19],[3,20],[3,21],[3,22],[3,23],[3,24],[3,25],
    [6,3],[9,3],
    [10,3],[10,9],[10,10],[10,11],[10,12],[10,13],[10,14],[10,15],[10,16],[10,17],[10,18],[10,19],[10,20],[10,21],[10,22],[10,23],[10,24],[10,25],
    ...
    [18,3],[19,3],[20,3],[21,3],[22,3],[23,3],[23,4],
    [24,4],[24,5],[24,6],[24,7],[24,8],[24,9],[24,10],[24,11],[24,12],[24,13],[24,14],[24,15],[24,16],[24,17],[24,18],[24,19],[24,20],[24,21],[24,22],[24,23],[24,24],[24,25]
  ];
  ...
  function makeSamSimpleShelterAllState() {
    ...
    const grid = Array.from({length:28}, (_,r) =>
      Array.from({length:28}, (_,c) => colonyConfigCellState('SAM',r,c,enabledSet))
    );
    const state = {
      era:'SAM',
      grid,
    ...
    for (let dr=0; dr<hall.h; dr++) {
      for (let dc=0; dc<hall.w; dc++) {
        state.grid[SAM_SIMPLE_SHELTER_ALL_HUB[0]+dr][SAM_SIMPLE_SHELTER_ALL_HUB[1]+dc] = 'hub';
      }
    }
    for (const [r,c] of SAM_SIMPLE_SHELTER_ALL_PATHS) state.grid[r][c] = 'road';
    for (const [r,c] of SAM_SIMPLE_SHELTER_ALL_SHELTERS) addBuilding('simpleShelter',r,c);
    for (const [r,c] of SAM_SIMPLE_SHELTER_ALL_DROP_PODS) addBuilding('dropPod',r,c);
  ...
  getPresetCatalog = function(...args) {
    ...
    const dropPodPreset = result?.find?.(p => p?.id === 'builtin:sam-drop-pods');
    ...
    if (
      ...
      const startingIndex = result.findIndex(p => p?.id === 'builtin:sam-simple-shelters');
      result.splice(startingIndex >= 0 ? startingIndex + 1 : result.length, 0, allExpansionPreset);
    }
