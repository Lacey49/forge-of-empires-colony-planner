...
(() => {
  ...
  const OFFICERS_37_ALL_QUARTERS = [[0,8],[0,12],[0,16],[4,0],[4,4],[4,8],[4,12],[4,16],[4,20],[4,24],[8,0],[8,4],[8,8],[8,12],[8,16],[8,20],[8,24],[12,0],[12,4],[12,8],[12,12],[12,16],[12,20],[12,24],[16,0],[16,4],[16,8],[16,12],[16,24],[20,0],[20,4],[20,8],[20,12],[20,24],[24,8],[24,12],[24,16]];
  const OFFICERS_37_ALL_CREW = [[16,16],[16,19]];
  ...
  function makeOfficers37AllState() {
    ...
    const grid = Array.from({length:28}, (_,r) =>
      Array.from({length:28}, (_,c) => colonyConfigCellState('SASH',r,c,enabledSet))
    );
    const state = {
      era:'SASH',
      grid,
    ...
    for (let dr=0; dr<hall.h; dr++) {
      for (let dc=0; dc<hall.w; dc++) grid[OFFICERS_37_ALL_HUB[0]+dr][OFFICERS_37_ALL_HUB[1]+dc] = 'hub';
    }
    ...
    for (const [r,c] of OFFICERS_37_ALL_QUARTERS) addBuilding('officersQuarters',r,c);
    for (const [r,c] of OFFICERS_37_ALL_CREW) addBuilding('simpleCrewQuarters',r,c);
    return state;
  }
  ...
  getPresetCatalog = function(...args) {
    ...
    if (!result.some(p => p?.id === 'builtin:sash-officers-all')) {
      ...
      const startingIndex = result.findIndex(p => p?.id === 'builtin:sash-officers');
