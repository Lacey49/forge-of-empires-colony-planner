...
(() => {
  ...
  const DEEP_88_ALL_MOVABLES = [
    [6, 0],
    [22, 0],
  ];
  const DEEP_88_ALL_PATHS = [
    [1, 24],
    [2, 11],
    [2, 12],
    [2, 13],
  ...
  function makeState({ enabled, hub, roads, deep = [], movable = [] }) {
    ...
    const grid = Array.from({ length: 28 }, (_, r) =>
      Array.from({ length: 28 }, (_, c) =>
        colonyConfigCellState("SAAB", r, c, enabledSet),
      ),
    );
    const state = {
      era: "SAAB",
  ...
  getPresetCatalog = function (...args) {
    ...
    if (!result.some((p) => p?.id === "builtin:saab-deep-all")) {
      ...
      const startingDeepIndex = result.findIndex(
        (p) => p?.id === "builtin:late",
      );
      result.splice(
        startingDeepIndex >= 0 ? startingDeepIndex + 1 : result.length,
        0,
        allPreset,
      );
    }
